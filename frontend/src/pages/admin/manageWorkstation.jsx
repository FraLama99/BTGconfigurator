...
import SideAdmin from "../../component/side/sideAdmin.jsx";
import api from "../../utlis/api.js";
import { useAuth } from "../../utlis/AuthContext.js";
import AddPresetForm from "../../component/admin/presetWK/AddPresetForm";
import EditPresetModal from "../../component/admin/presetWK/EditPresetModal";
import DeletePresetModal from "../../component/admin/presetWK/DeletePresetModal";
import PresetsTable from "../../component/admin/presetWK/PresetsTable";
...
const ManageWorkstation = () => {
  ...
  const handleCreateSubmit = async (formData) => {
    ...
    try {
      ...
      // Chiudi il form dopo il salvataggio (opzionale)
      // setShowAddForm(false); // Commentato per lasciarlo aperto se preferisci
  ...
  const handleEditClick = (preset) => {
    ...
    const safePreset = {
      ...
      category: preset.category || "workstation",
  ...
  const handleEditSubmit = async (updatedPreset) => {
    try {
      ...
      setSuccess("Configurazione workstation aggiornata con successo!");
      fetchPresetMachines();
  ...
  const confirmDelete = async () => {
    try {
      ...
      setSuccess("Configurazione workstation eliminata con successo!");
      fetchPresetMachines();
      setTimeout(() => setSuccess(null), 3000);
    } catch (error) {
  ...
  const calculateBasePrice = () => {
    ...
    // Se non ci sono componenti selezionati, restituisci solo il costo di assemblaggio
    if (!hasAnyComponent) {
      return "250.00"; // Solo assemblaggio
    }
  ...
  return (
    <div className="d-flex">
      ...
      >
        <h1 className="mb-4">Gestione Configurazioni Workstation</h1>
        ...
        <Card className="mb-4">
          ...
          >
            <div>
              ...
              ></i>
              Aggiungi Nuova Configurazione Workstation/Office
            </div>
            <Button
        ...
        <Card>
          <Card.Header as="h5">
            ...
            Configurazioni Workstation/Office Esistenti
          </Card.Header>
          <Card.Body>
            <PresetsTable
              presets={presetMachines.filter(
                (p) => p.category === "office" || p.category === "workstation"
              )}
              loading={loading}
