...
const PresetsTable = ({ presets, loading, onEdit, onDelete }) => {
  ...
  // Filtra le configurazioni per mostrare solo office e workstation
  const filteredPresets = presets.filter(
    (preset) =>
      preset.category === "office" || preset.category === "workstation"
  );
  ...
  if (!filteredPresets.length) {
    return (
      <Alert variant="info">
        Nessuna configurazione workstation/office disponibile. Crea la tua prima
        ...
        <tbody>
          {filteredPresets.map((preset) => (
            <tr key={preset._id}>
              ...
              <td>
                <Badge bg={preset.category === "office" ? "secondary" : "info"}>
                  {preset.category}
                </Badge>
              </td>
              <td>
